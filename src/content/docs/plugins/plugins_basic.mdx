---
title: "Basic plugins"
description: "This document describes the @asicupv/paella-basic-plugins package."
---
...
> **Note:** the fullscreen mode is not supported on iOS devices. If you have a web page with a list of several players, you can emulate the fullscreen mode using CSS as described in the following document: [Fullscreen Fallback for iOS Devices](/reference/fullscreen_fallback_ios_devices).
...
### es.upv.paella.audioEnhancement & es.upv.paella.audioEnhancementButton

These two plugins are designed to work together:

- audioEnhancement: an AudioProcessor-type plugin that enhances the audio by adding a compressor filter and a gain filter. It is designed to enhance the audio in videos where the sound levels are too low.
- audioEnhancementButton: a Button-type plugin that controls the activation of audioEnhancement.

```json
{
    "es.upv.paella.audioEnhancement": {
        "enabled": true
    },
    "es.upv.paella.audioEnhancementButton": {
        "enabled": true,
        "side": "right"
    }
}
