...
Note that the `@asicupv/paella-core` package includes several layout plugins that can handle multiple video streams. This document is intended for developers who want to create their own layout plugins. If you want to use the existing plugins, you can find more information in the [`paella-core` included plugins reference](/plugins/plugins_core) document.
