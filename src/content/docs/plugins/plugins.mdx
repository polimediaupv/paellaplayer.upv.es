...
- [`@asicupv/paella-core`](/plugins/plugins_core)
- [`@asicupv/paella-basic-plugins`](/plugins/plugins_basic)
- [`@asicupv/paella-video-plugins`](/plugins/plugins_video)
- [`@asicupv/paella-slide-plugins`](/plugins/plugins_slides)
- [`@asicupv/paella-zoom-plugin`](/plugins/plugins_zoom)
- [`@asicupv/paella-webgl-plugins`](/plugins/plugins_webgl)
...
- [ButtonPlugin](/plugins/button_plugins): Adds buttons to the player toolbar.
- [PopUpButtonPlugin](/plugins/pop_up_button_plugins): Adds a button that opens a pop up with content defined by the plugin.
- [MenuButtonPlugin](/plugins/menu_button_plugins): Adds a button that opens a menu with options defined by the plugin.
- [TableInfoPopUpPlugin](/plugins_table_info_pop_up_plugins): Adds a button that opens a pop up with a table of information defined by the plugin.
- [DataPlugin](/plugins/data_plugins): Adds a plugin that allows to read and write data using the Paella Player `Data` API (for example, to communicate with a backend).
- [VideoPlugin](/plugins/video_plugins): Adds a plugin that allows to extend the video formats supported by Paella Player.
- [VideoLayout](/plugins/video_layout): Adds a plugin that allows to define the layout of the video in the player.
- [CanvasPlugin](/plugins/canvas_plugins): Adds a plugin that allows to define the video representation in the player.
- [EventLogPlugin](/plugins/event_log_plugins): Adds a plugin that allows to log events in the player.
...
- **`getPluginModuleInstance()`**: Returns the instance of the [plugin module](/plugins/plugin_modules) that contains the plugin. You should define your own plugin module to group your plugins, but in the following example we are using the `PaellaCorePlugins` module:
...
- **`preload()`**: This method is called when the plugin instance is created, just after loading the plugin configuration. This is a synchronous method, because whatever we do within this function must be lightweight tasks: all plugins added to the player's initParams object will call this function, regardless of whether the plugin is ultimately enabled or not.

```js
preload() {
  console.log("Preloading plugin");
}
```
