---
title: "Events"
description: "The Paella player instance contains an API that allows you to listen to events. The events are triggered by the player and can be used to perform actions when certain events occur. In this page, you will learn how to use the event API in your plugins, and how to use the event API with other plugins"
---

Paella Player relies on an asynchronous API because it's designed to work with a variety of media sources and formats that may not be available immediately. For example, if you click the play button, maybe the video is not loaded yet, or the video is not ready to play.

For that reason, the `paella-core` library also uses a set of events to notify the player when certain actions occur. These events are triggered by the player and can be used to perform actions when certain events occur.
In this page, you will learn how to use the event API in your plugins, and how to use the event API with other plugins.
