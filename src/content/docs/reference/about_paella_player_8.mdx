...
Initially, until **Paella Player 6**, the architecture of the player was monolithic. However, with the release of 
...
![time markers](about_paella_player_8/paella-ecosystem.svg)
...
To handle keyboard events, it is recommended that you use any of the available plugins. The @asicupv/paella-extra-plugins package provides a plugin that implements both basic handling of typical keyboard events and a help menu listing the options supported by the plugin.
