...
description: StreamProvider is the component in charge of managing the playback and synchronization of video streams.  It is also in charge of managing soft trimming.
---
...
- If we want to perform actions on video playback that do not generate events. The one in charge of triggering playback events (play, pause, volume change, seek, etc) is [VideoContainer](/reference/video_container), so if we use StreamProvider to perform these actions, no events will be triggered.
...
StreamProvider is accessed through the [VideoContainer](/reference/video_container):
...
`async setTrimming({ enabled, start, end })`: Configures video trimming. This function generates the `Events.TIMEUPDATE` event, so that if the video is paused, all event watchers can recalculate the video properties. This is the only function of the Stream Provider trimming API that generates events. The rest of the trimming events are triggered by the [Video Container](/reference/video_container).
...
The multiple audio control functions allow you to change the audio track of the main audio stream, provided that the [video plug-in](/plugins/video_plugins) supports it. All multi audio functions work with instances of objects of the `AudioTrackData` class. More information on videos with multiple audio tracks can be found in [this document](/reference/multiaudio).
...
These functions provide managed access to the input and output nodes for the Web Audio API. For further information, see the documentation on [Web Audio API](/reference/stream_provider_web_audio_api)

`get audioContext(): AudioContext`: Returns the Web Audio API context, associated with the main audio stream.

`get audioSourceNode(): MediaElementAudioSourceNode`: Returns the Web Audio API source node.

`get audioDestinationNode()`: Returns the Web Audio API destination node.

`async reloadAudioProcessors()`: Reload the Web Audio API graph.
...
It is possible to control the maximum desynchronization we allow through the paella-core setting. The default value if nothing is specified is 0.2 seconds:
