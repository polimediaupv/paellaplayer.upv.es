...
As a general rule, it is not possible to customize the icons of third-party library plugins, as it is the plugin's responsibility to set its icon. However, if we want our plugin to support icon customization, we can use the following APIs from the `Paella` class instance:

- `Paella.getCustomPluginIcon(pluginName,iconName)`: Get the `iconName` SVG data for `pluginName` plugin.
- `Paella.addCustomPluginIcon(pluginName,iconName,svgData)`: Set the `iconName` SVG data for `pluginName` plugin.
- `Paella.removeCustomPluginIcon(pluginName,iconName)`: Remove the `iconName` SVG data for `pluginName` plugin.
- `Paella.requestedCustomIcons`: Returns the list of requested custom icons.
...
If the icon has not been customized, the `getCustomPluginIcon` function will return `null`, and therefore the icon defined by the plugin will be assigned. Otherwise the custom plugin will be used.

Because support for icon customization depends on each plugin implementing it via the above APIs, each third-party plugin library must detail whether the plugins it exports are supported, and document the corresponding names of each icon. You can check the documentation of the Paella Player official plugin libraries for more information about the icons that can be customized:
...
Some of the icons that do not belong to any plugin can be customised through the icon customisation API, using the name of the library package as the plugin identifier.

- `@asicupv/paella-core`:
	* `playPreview`: modifies the play icon in the preview container that appears before loading the player.


```ts
...
import CustomPlayIcon from "./MyCustomPlayIcon.ts";
...
