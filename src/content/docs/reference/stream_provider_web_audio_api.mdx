---
title: Stream Provider Web Audio API
description: Exposes a low-level audio processing pipeline built on the Web Audio API] to tap into the video's audio stream, apply custom processing, or capture audio data in real time.
---
