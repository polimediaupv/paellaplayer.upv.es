---
title: Video layout
description: The video layout is the way the different video streams are displayed in the player container
---

The video layout is the way the different video streams are displayed in the player container. The different video layouts can be defined using plugins. In this document we will explain how to create a video layout plugin, and how to configure it.

Note that the `@asicupv/paella-core` package includes several layout plugins that can handle multiple video streams. This document is intended for developers who want to create their own layout plugins. If you want to use the existing plugins, you can find more information in the [`paella-core` included plugins reference](/reference/plugins_core) document.

## Define a VideoLayout plugin

```javascript
import { VideoLayout } from '@asicupv/paella-core';
...
From Paella Player 2 (`@asicupv/paella-core` >= 2.0) this kind of layouts are deprecated, and it is recommended to use the dynamic layouts instead. The static layouts are still available for backward compatibility, but they will be removed in future versions of Paella Player.
