...
The default implementation of the mp4 plugin in `paella-core` does not support multi quality streams. To implement this functionality, it is recommended to use the [HLS video plugin](/reference/video_plugin_hls) included in `@asicupv/paella-video-plugins`. But if you want to use mp4 videos with multi quality streams, you can use the [MP4 multi quality plugin](/reference/video_plugin_mp4_multi_quality), also available in `@asicupv/paella-video-plugins`.
...
The use of `enable/disable` video API has been removed due to a very unstable behaviour. For this reason, mp4 streams are always played even if they are hidden. The malfunction is reproducible in almost all browsers. Problems occur when trying to remove and re-add the video tag to the DOM tree. We recommend using the [HLS video plugin](/reference/video_plugin_hls), which although it does not have these functions implemented, will save bandwidth if variable bitrate streams are configured.
