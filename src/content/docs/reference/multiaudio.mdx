...
`paella-core` supports multiple audio tracks through the [`StreamProvider`](/reference/stream_provider) APIs. However, the underlying video technology being used to play the stream must also support multiple audio tracks. For example, the audio track APIs for `.mp4` files are supported by some browsers, but not all, and for that reason the [`mp4` video plugin](/plugins/video_plugin_mp4) does not support multiple audio tracks: the player behaviour would not be the same in all browsers. However, the `hls` video format does allow the use of video streams with multiple audio tracks, so the [hls_video_plugin.md](/plugins/video_plugin_hls) does implement this feature.
...
- [HLS video plugin](/plugins/video_plugin_hls) for information about multiaudio in HLS.
