---
title: Log messages
description: The message logging system of Paella Player allows to log messages in the console depending on the current log level
---
...
```javascript
import { Paella, LOG_LEVEL } from '@asicupv/paella-core';

const initParams = { ... };

const playerInstance = new Paella('playerContainer', initParams);
...
The `logLevel` parameter is case insensitive, so you can also use:
