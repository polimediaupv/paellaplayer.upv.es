---
title: Localization
description: In this section we will learn how to localize the player
---
...
```javascript
import { setLanguage, getLanguage, translate } from '@asicupv/paella-core';
...
```javascript
import { addDictionary } from '@asicupv/paella-core';
```
...
The default language of the player is set in the [configuration file](/reference/configuration). If we want to change this value to a different one, we have to do it also in the `loadDictionaries` function. The following example shows how to add a dictionary and also set the default language from the browser language:

```javascript
import { addDictionary, setLanguage } from '@asicupv/paella-core';
...
const initParams = {
  ...
...
For more information about Paella Player initialization, check [this document](/reference/initialization).
...
It's possible to extend Paella Player dictionaries through plugins. Plugins that add user interaction are able to add translation text strings. These are the descendant plugins of the [UserInterfacePlugin](/reference/user_interface_plugin) class.
...
For more information, see the documentation for the plugin types descended from [UserInterfacePlugin](/reference/user_interface_plugin):
...
- [VideoLayout](/reference/video_layout_plugins)
...
The `getDictionaries()` function is also available for classes that inherit from `PluginModule`. This way, it is possible to unify all dictionaries of a plugin module within the module definition:
