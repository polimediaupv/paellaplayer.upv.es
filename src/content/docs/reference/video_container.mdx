...
description: The video contianer is responsible for managing the video layout and playback controls.
---

The VideoContainer component performs three tasks:

- **Obtain compatible video layouts:** from the [video manifest](/reference/video_manifest), and taking into account the configuration of the [video layout plugins](/reference/video_layout), a list of layouts that are compatible with the current video configuration will be generated. It also provides the necessary API to retrieving the supported layouts and activate them.
...
<blockquote>
    **Note:** in Paella Player 8 (`@asicupv/paella-core` >= 2.0) you can access to the basic playback functions through the `Paella` instance. These functions are available in the `Paella` instance for simplicity, but is still valid to use the `videoContainer` instance to access them. In fact, the playback functions of the `Paella` instance are implemented as a wrapper of the `videoContainer` functions.
</blockquote>
...
Content identifiers, hereafter contentId, identify a video layout configuration. They are configured in the `config.json` file, in the configuration options of the layout plugins, as indicated in [this document](/reference/video_layout). The `validContent` array contains the different layout settings that can be applied, depending on the `content` section of the [video manifest](/reference/video_manifest#content):
...
For more information, see [the documentation about video layouts](/reference/video_layout).
...
This code is extracted from the `en.upv.paella.layoutSelector` plugin, which is one of the plugins available in the `@asicupv/paella-basic-plugins` package. We have left only the parts of the code relevant to list the available layouts and activate it. For more information, you can check the documentation about [menu button plugins](/reference/menu_button_plugins).
...
To get the actual current time values while keeping trimming enabled, we can use the APIs of [stream provider](/reference/stream_provider).
...
- video layout: store the las video layout set. This setting was stored in previous versions of paella-core, but from 1.19 is configurable. This property can be configured to be saved globally (for all videos) or specific to the current video identifier.
- last known time: store the last known time for each video identifier. You can set the number of seconds remaining in the video so that the video starts playing from the beginning. For example, if there are 5 seconds left in the video, the next time the video loads it will start from the beginning.
...
The [preferences API](/reference/preferences) is used to store these settings. The values of these properties are always saved, but will only be restored if the option is activated in the configuration, except for the last known time instant, which is only saved if the option is active.
...
Check the documentation about [styling Paella Player](/reference/stiling_css_variables) to learn how to change the CSS styles of the video container.

### Dynamic layouts

In dynamic layouts, the alignment of the video streams is configured via the configuration file, with the `landscapeVerticalAlignment` and `portraitHorizontalAlignment` properties. For more information, see the section on [dynamic layouts](/reference/video_layout).
