...
- **Obtain compatible video layouts:** from the [video manifest](/reference/video_manifest), and taking into account the configuration of the [video layout plugins](/plugins/video_layout), a list of layouts that are compatible with the current video configuration will be generated. It also provides the necessary API to retrieving the supported layouts and activate them.
...
Content identifiers, hereafter contentId, identify a video layout configuration. They are configured in the `config.json` file, in the configuration options of the layout plugins, as indicated in [this document](/plugins/video_layout). The `validContent` array contains the different layout settings that can be applied, depending on the `content` section of the [video manifest](/reference/video_manifest#content):
...
For more information, see [the documentation about video layouts](/plugins/video_layout).
...
This code is extracted from the `en.upv.paella.layoutSelector` plugin, which is one of the plugins available in the `@asicupv/paella-basic-plugins` package. We have left only the parts of the code relevant to list the available layouts and activate it. For more information, you can check the documentation about [menu button plugins](/plugins/menu_button_plugins).
...
In dynamic layouts, the alignment of the video streams is configured via the configuration file, with the `landscapeVerticalAlignment` and `portraitHorizontalAlignment` properties. For more information, see the section on [dynamic layouts](/plugins/video_layout).
