...
Data plugins provide an interface for communication with the backend that can be extended through the use of [plugins](/reference/plugins).
...
export default class MyBackendDataPlugin extends DataPlugin {
    ...
    get name() {
        return super.name || "es.upv.paella.myBackendDataPlugin";
    }
