...
It is implemented from [hls video plugin](/reference/video_plugin_hls). The difference is that this plugin responds to streams labeled `hlsLive` from [video manifest](/reference/video_manifest). In addition, the hls live plugin does not support setting the initial video quality, as this setting usually gives problems with low latency videos.
...
If you use [wowza streaming server](https://www.wowza.com/), you can follow this [guide](/guides/wowza_low_latency) to configure HLS Low latency.


## More information and FAQ

See the document about the [hls video plugin](/reference/video_plugin_hls) for more information.
