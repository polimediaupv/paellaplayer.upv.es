...
The colours of interface elements can be defined in a granular way (e.g. the background colour of a button) or in a global way. In the default `paella-core` styles there are a number of generic colours, which are then used as defaults in the other variables that control the colours in a granular way. Generally, it will be enough to modify the global colours, but if you need to modify a specific variable you can do that too.

TODO:

--button-color, --icon-color, --icon-fill-color, --icon-stroke-color, --button-background, --button-background-hover, --button-background-active

### Set colors for a specific button

It is also possible to specify specific colours for certain `ButtonPlugin` plugins (or their derivatives). Plugins of type `ButtonPlugin` initialise the `name` attribute of the button with the plugin identifier:
